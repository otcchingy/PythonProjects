...
class StringEnumerator(Enumerator):
    ...
    def __init__(self, script: str = None, **kwargs):
        ...
        if (not self._enum_type) or self._enum_type == 'line':
            ...
            self._e_walker = DefaultWalker(script)

        super().__init__(script)

    def set_pointer(self, index):
        if self._enum_type == 'word':
            return self._e_walker.set_pointer(index)
        return self.set_pointer(index)

    def add_item(self, item):
        if self._enum_type == 'word':
            return self._e_walker.add_item(item)
        return super().add_item(item)

    def add_all_items(self, items):
        if self._enum_type == 'word':
            return self._e_walker.add_all_items(items)
    ...
    def get_items(self):
        if self._enum_type == 'word':
            return self._e_walker.get_items()
        return super().get_items()

    def get_stop_index(self):
        if self._enum_type == 'word':
            return self._e_walker.get_stop_index()
        return super().get_stop_index()

    def get_start_index(self):
        if self._enum_type == 'word':
            return self._e_walker.get_start_index()
        return super().get_start_index()

    def get_current_item_number(self):
        if self._enum_type == 'word':
            return self._e_walker.get_current_item_number()
        return super().get_current_item_number()

    def peek_next_index(self, not_in=''):
        if self._enum_type == 'word':
            c_index = self._e_walker.get_current_item_number() + 1
            if not_in != '':
                while str(self._e_walker.peek(c_index)) in not_in and c_index <= self._e_walker.get_stop_index():
                    c_index += 1

            if c_index <= self._e_walker.get_stop_index():
                return c_index
            return -1
        else:
    ...
    def peek_next_not_in(self, line: str):
        ...
        if index > -1:
            if self._enum_type == 'word':
                return self._e_walker.peek(index)
    ...
    def peek_next(self):
        if self._enum_type == 'word':
            return self._e_walker.peek_next()
        return super().peek_next()

    def peek_previous(self):
        if self._enum_type == 'word':
            return self._e_walker.peek_previous()
    ...
    def has_next(self):
        if self._enum_type == 'word':
            return self._e_walker.has_next()
    ...
    def has_previous(self):
        if self._enum_type == 'word':
            return self._e_walker.has_previous()
        else:
