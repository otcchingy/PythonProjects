...
class MyvarpScriptInterpreter(MyvarpScriptReader):
    ...
    @staticmethod
    def get_dict_value(data: dict):
        return list(data.values())[0]

    @staticmethod
    def get_dict_key(data: dict):
    ...
    def __interpret(self, expression: list):
        ...
        for i in range(len(self.__current_expression_data)):
            key_value: dict = self.__current_expression_data[i]
            key = self.get_dict_key(key_value)

            print("current word : " + str(key_value))
    ...
    def __interpret_assignment(self):
        ...
        if self._is_expression_line_complete():
            ...
            process = Process('assignment', self.get_dict_value(data[0]), '=',
                              "".join(map(lambda x: self.get_dict_value(x), data[2:])), None, 'ongoing')
    ...
    def __interpret_next_expression_as_data(self):
        ...
        if 'identifier' in self.get_dict_key(self.__get_interpretable_expression_data()[0]):
            key = self.get_dict_value(self.__get_interpretable_expression_data()[0])
            return self.get_data_from_memory(key)
        else:
            value = self.get_dict_value(self.__get_interpretable_expression_data()[0])
            logging.debug(value)
